// AeroNav — Finance page (qui paye quoi)
...
function FinancePage({ variant }) {
  const fin = window.computeFinance(variant);
  ...
  // Sort pilots by total descending
    ...
    <div style={{padding: 16, height: "100%", display: "flex", flexDirection: "column", gap: 14, overflow: "auto", background: "var(--surface)"}}>
      ...
      <div className="card" style={{padding: "14px 16px", display:"grid", gridTemplateColumns: "1.4fr 1fr 1fr 1fr", gap: 16, alignItems:"center"}}>
        ...
        <BreakdownStat label="Heures de vol (coût)" value={fmtEUR(fin.totals.flightCost)}
          sub={`${fin.totals.hours.toFixed(1)}h × tarif horaire`}
          color="var(--aero-blue)" />
        <BreakdownStat label="Taxes aérodromes" value={fmtEUR(fin.totals.taxesCost)}
          sub="atter. + parking, × 4 avions"
          color="var(--aero-amber)" />
        <BreakdownStat label="Pilotes facturés" value={`${pilotBills.length}/${window.PEOPLE.filter(p=>p.rolePref==="CDB").length}`}
      ...
      <div style={{display:"grid", gridTemplateColumns: "repeat(auto-fill, minmax(360px, 1fr))", gap: 12, alignContent:"start"}}>
        {pilotBills.map(bill => {
          const p = window.personById(bill.personId);
          if (!p) return null;
          ...
          // Variance bar : centered at 50%, extends left/right relative to maxAbsDelta
          const barPct = Math.abs(delta) / maxAbsDelta * 50; // up to 50% of bar width on each side
          return (
            <div key={p.id} className="card" style={{overflow:"hidden", display:"flex", flexDirection:"column"}}>
              ...
              {isOpen ? (
                <BillDetail bill={bill} variant={variant} />
              ) : null}
            </div>
          );
        })}
        ...
        {unassigned.length > 0 ? (
          <div className="card" style={{padding: "14px 16px", borderColor: "var(--aero-red)", background:"#fff7df"}}>
            ...
            <div style={{fontSize: 10.5, color:"var(--ink-3)", marginTop: 8, fontStyle:"italic"}}>
              Affectez un CDB sur ces branches dans l'onglet Voyage pour répartir ces coûts.
            </div>
          </div>
        ) : null}
      </div>
...
function BillDetail({ bill, variant }) {
  return (
    <div style={{padding: "0 0 4px"}}>
      <table className="table" style={{fontSize: 11}}>
        ...
        <tbody>
          {bill.items.map((it, i) => {
            const ac = window.acById(it.acId);
            const m = window.AC_MODELS[ac.model];
            return (
              <tr key={i}>
                <td>
                  <span style={{
                    width: 18, height: 18, borderRadius:"50%", background: "var(--ink)", color:"#fff8df",
                    display:"inline-flex", alignItems:"center", justifyContent:"center",
                ...
                <td style={{textAlign:"right"}} className="mono">{window.fmtHr(it.durMin)}</td>
                <td style={{textAlign:"right"}} className="mono">{fmtEUR(it.flightCost)}</td>
                <td style={{textAlign:"right"}} className="mono">
                  {it.landingTax + it.parkingTax > 0 ? (
                    <span title={`Atter. ${it.landingTax}€ · Park. ${it.parkingTax}€`}>
                      {fmtEUR(it.landingTax + it.parkingTax)}
                    </span>
          ...
          <tr>
            ...
            <td style={{textAlign:"right", borderBottom: 0, paddingTop: 10}} className="mono"><b>{window.fmtHr(bill.hours * 60)}</b></td>
...
function BreakdownStat({ label, value, sub, color }) {
  return (
    <div style={{padding: "0 16px", borderLeft: "1px solid var(--hairline-soft)"}}>
      ...
      {sub ? <div style={{fontSize: 10.5, color:"var(--ink-3)", marginTop: 2}}>{sub}</div> : null}
    </div>
  );
}
